...
export const createTodo = async (req, res) => {
  try {
    const { title, description, createdBy, completed = false } = req.body;
...
export const getAllTodos = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .send({ success: false, message: "Error fetching todos" });
  }
};
//delete todo
export const deleteTodo = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .send({ success: false, message: "Error when deleting todo" });
  }
};
//update todo
export const updateTodo = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .send({ success: false, message: "Error when updating todo" });
  }
};
